...
public class MNISTAnimalsClassifier {
    private static final long SEED = 42;
    private static final Random RANDOM_NUMBER_BY_SEED = new Random(SEED);
    private static final int NUMBER_OF_UNIQUE_DATA_BY_LABEL = 18;
    ...
    /**
     * Main Animal Classifier method
     * Start Animal data classification and neural network training
     */
    public void classifyAnimals() {
        ParentPathLabelGenerator labelMaker = new ParentPathLabelGenerator();

        SplitAndFilter splitAndFilter = categorizeAnimalImages(labelMaker);
        ...
        MNISTAnimalModelTrainer mnistAnimalModelTrainer = new MNISTAnimalModelTrainer();
        ...
        mnistAnimalModelTrainer.trainModel(trainModelData);
    }

    /**
     * Scan resources for pictures of unique animals and their labels
     * (unique species - bear, deer, duck, turtle)
     * <p>
     ...
     */
    private SplitAndFilter categorizeAnimalImages(ParentPathLabelGenerator labelMaker) {
        File animalLabels = new File("NeuralNetworks/src/main/resources/animals");
        FileSplit fileSplit = new FileSplit(animalLabels, NativeImageLoader.ALLOWED_FORMATS, RANDOM_NUMBER_BY_SEED);
        int numExamples = toIntExact(fileSplit.length());
        BalancedPathFilter pathFilter = new BalancedPathFilter(RANDOM_NUMBER_BY_SEED, labelMaker, numExamples, NUMBER_OF_UNIQUE_ANIMALS, NUMBER_OF_UNIQUE_DATA_BY_LABEL);
